...
window.onload = function () {
  ...
  var cf7a_version = cf7a_settings.version; // disable cf7 refill on load
  ...
  var browserFingerprint = function browserFingerprint() {
    var _Intl$DateTimeFormat$, _navigator$platform, _navigator$hardwareCo, _ref, _navigator$deviceMemo, _navigator$appVersion, _ref2; // as reference https://developer.mozilla.org/en-US/docs/Web/API/Navigator/hardwareConcurrency


    var ua = navigator.userAgent; // holds the object with the tested props
   ...
   * @returns A new input element with the type of hidden, name of the key, and value of the value.
   */
    ...
    (function () {
      ...
      try {
        var _loop = function _loop() {
          ...
          var hiddenInputsContainer = wpcf7Form.querySelector('form > div'); // 1) Standard bot checks

          var bot_fingerprint_key = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'bot_fingerprint]'); // 2) Bot fingerprint extra checks

          var bot_fingerprint_extra = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'bot_fingerprint_extras]'); // 3) Language check

          var language_checks_enabled = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + '_language]'); // how append bot fingerprint into hidden fields

          var append_on_submit = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'append_on_submit]'); // how append bot fingerprint into hidden fields

          var cf7a_version_input = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'version]'); // get the fake field and skip it

          if (wpcf7Form.querySelector('form').getAttribute('autocomplete')) return "continue"; // then set the cf7 antispam version field

          cf7a_version_input.setAttribute("value", cf7a_version); // fingerprint browser data

          var tests = browserFingerprint();

          if (bot_fingerprint_key) {
            // 1.0 hijack the value of the bot_fingerprint
            bot_fingerprint_key.setAttribute("value", bot_fingerprint_key.getAttribute("value").slice(0, 5)); // then append the fields on submit
            ...
            if (!append_on_submit || tests.isIos || tests.isIE) {
              ...
              formElem.addEventListener('formdata', function (e) {
                ...
                for (var _key in tests) {
                  data.append(cf7a_prefix + _key, tests[_key]);
                }
          ...
          if (bot_fingerprint_extra) {
            ...
            var activity = function activity(e) {
              var bot_activity = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'activity]');
