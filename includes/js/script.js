...
window.onload = function () {
  ...
  var cf7a_prefix = cf7a_settings.prefix; // disable cf7 refill on load

  wpcf7.cached = parseInt(cf7a_settings.disableReload) === 0 && wpcf7.cached; // if is cached disable reload

  var _testTouch = function testTouch() {
    if ("maxTouchPoints" in navigator) {
      _testTouch = navigator.maxTouchPoints > 0;
    } else if ("msMaxTouchPoints" in navigator) {
      _testTouch = navigator.msMaxTouchPoints > 0;
    } else {
      var mQ = window.matchMedia && matchMedia("(pointer:coarse)");

      if (mQ && mQ.media === "(pointer:coarse)") {
        _testTouch = !!mQ.matches;
      } else if ('orientation' in window) {
        _testTouch = true; // deprecated, but good fallback
      } else {
        // Only as a last resort, fall back to user agent sniffing
        var UA = navigator.userAgent;
        _testTouch = /\b(BlackBerry|webOS|iPhone|IEMobile)\b/i.test(UA) || /\b(Android|Windows Phone|iPad|iPod)\b/i.test(UA);
      }
    }
  ...
  var browserFingerprint = function browserFingerprint() {
    ...
    if (tests.isIos || tests.isAndroid) tests.touch = _testTouch();
    return tests;
  };
    ...
    (function () {
      ...
      try {
        var _loop = function _loop() {
          ...
          var append_on_submit = hiddenInputsContainer.querySelector('input[name=' + cf7a_prefix + 'append_on_submit]'); // fingerprint browser data
